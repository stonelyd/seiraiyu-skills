...
5. Open the app with `agent-browser open <url>` and confirm it loads
6. Take an initial screenshot: `agent-browser screenshot e2e-screenshots/00-initial-load.png`
...
5. **Take a screenshot** — save to a descriptive path under `e2e-screenshots/` organized by journey (e.g., `e2e-screenshots/profile-creation/03-form-submitted.png`)
6. **Analyze the screenshot** — use the Read tool to view the screenshot image. Check for visual correctness, UX issues, broken layouts, missing content, error states
7. Check `agent-browser console` and `agent-browser errors` periodically for JavaScript issues
